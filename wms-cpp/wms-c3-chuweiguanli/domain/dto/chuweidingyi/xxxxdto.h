...
//导出
class daochudto : public oatpp::DTO
{
	DTO_INIT(daochudto, DTO);
	
};


//导入
class daorudto : public oatpp::DTO
{
	DTO_INIT(daorudto, DTO);
	DTO_FIELD(Boolean, panduan);
	DTO_FIELD_INFO(panduan)
	{
		info->description = ZH_WORDS_GETTER("daoru.dr.panduanzhi");
	}
